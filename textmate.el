...
(defun textmate-go-column (direction arg)
  ...
  (let* ((orig-line (line-number-at-pos))
         ...
         (textmate-word-boundary-regex (format "[^%s][%s]" textmate-word-characters textmate-word-characters))
         (word-start (save-excursion
                       (forward-char)
                       (if (search-backward-regexp textmate-word-boundary-regex nil t)
                           (forward-char)
                         (search-backward-regexp (format "^[%s]" textmate-word-characters)))
                       (current-column)))
         (matching-regex (if (= word-start 0)
                             (format "^[%s]" textmate-word-characters)
                           (format "^.\\{%d\\}[^%s][%s]" (- word-start 1) textmate-word-characters textmate-word-characters)))
         (do-search (if (= direction 1)
                        (lambda ()
                          (move-to-column (1+ word-start))
                          (search-forward-regexp matching-regex nil t))
                      (lambda ()
                        (move-to-column word-start)
                        (search-backward-regexp matching-regex nil t)))))
    (funcall do-search)
